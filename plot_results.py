...
df = pd.DataFrame(columns=["dataset", "network_precision", "scheduler", "idx", "mean", "std"])
        ...
        for scheduler in data[precision][dataset].keys():
            ...
            for idx, trial in enumerate(data[precision][dataset][scheduler]):
                if trial is not None:
                    ...
                    grouped_trial_df = grouped_trial_df.append({"idx": 0, "loss": 1}, ignore_index=True)  
...
del df_quant
df_mnist_quant['mean_rolling'] = df_mnist_quant.iloc[:,4].rolling(window=20, min_periods=1).mean()
df_mnist_quant['std_rolling'] = df_mnist_quant.iloc[:,5].rolling(window=20, min_periods=1).mean()
df_mnist_quant = df_mnist_quant.dropna()
df_fmnist_quant['mean_rolling'] = df_fmnist_quant.iloc[:,4].rolling(window=20, min_periods=1).mean()
df_fmnist_quant['std_rolling'] = df_fmnist_quant.iloc[:,5].rolling(window=20, min_periods=1).mean()
df_fmnist_quant = df_fmnist_quant.dropna()
df_dvs_quant['mean_rolling'] = df_dvs_quant.iloc[:,4].rolling(window=5, min_periods=1).mean()
df_dvs_quant['std_rolling'] = df_dvs_quant.iloc[:,5].rolling(window=5, min_periods=1).mean()
...
for precision_idx, precision in enumerate(data.keys()):
    for dataset_idx, dataset in enumerate(data[precision].keys()):
        ...
        # Plot mean values
        sns.lineplot(data=df_, x="idx", y=col_name, hue="scheduler", ax=ax[dataset_idx], linewidth=2.5, palette=palette) # alpha=0.95
