...
df = pd.DataFrame(columns=["dataset", "network_precision", "scheduler", "test_set_accuracy_best", "test_set_accuracy_mean", "test_set_accuracy_std"])
for precision in data.keys():
    for dataset_idx, dataset in enumerate(data[precision].keys()):
        for scheduler in data[precision][dataset].keys():
            ...
            df = df.append({'dataset': dataset, 'network_precision': precision, 'scheduler': scheduler, 'test_set_accuracy_best': max(test_set_accuracy_values), 'test_set_accuracy_mean': np.mean(test_set_accuracy_values), 'test_set_accuracy_std': np.std(test_set_accuracy_values)}, ignore_index=True)
